...
description: 讀取靜態 HTML 雛形頁面與程式規格書，經由結構化 UI Schema 中介，產生符合規範的 React 18 + Vite + React-Bootstrap 前端程式碼（通用列表/表單範本）。當使用者提供 HTML 雛形、要求產生 React 頁面或元件時使用。
---

# Prototype to React（Phase 1）

以資深 React 18 前端工程師的角度，將已確認的 HTML 雛形頁面與後端程式規格書，轉換為符合專案架構規範的**可直接使用的 React 程式碼**。
...
├── index.html / *index*.html  ← 主頁/列表頁相關雛形（至少 1 份，非限定檔名）
├── form.html / *form*.html    ← 表單頁相關雛形（若需求包含表單則必須）
├── 其他 *.html                ← 與 index 頁相關之區塊頁、彈窗頁、子頁（可一併讀取）
└── mock-data.js            ← 模擬資料（Initial 模式可不存在，需自動建立）
...
1. 不限制只能讀取 `index.html` 與 `form.html`，凡是使用者提供且與 `index` 頁相關的 HTML 都必須納入分析
2. 若同一功能有多份雛形（如 `index.html`、`index-list.html`、`index-modal.html`），需先整併為單一 UI Schema 再產碼
3. 若未提供 `form` 類 HTML，但需求明確包含表單流程，必須先提問補齊，不得自行臆測欄位

---

## Reuse 與增量更新模式（強制）

此 Skill 必須可重複使用。當使用者在「已產出程式碼」後再次提出更新需求時，採用**增量更新**，不得全量重生。
...
（附帶說明）src/Router/index.tsx    ← 路由集中管理（Route 僅能定義於此）
（附帶說明）src/App.tsx             ← 僅保留入口與必要 Provider，不可放 Route 宣告
```

---

## Router 與 App.tsx 職責規範（強制）

1. 所有頁面 Route 必須集中管理於 `src/Router/index.tsx`，不可分散於各頁面或 `App.tsx`
2. `src/App.tsx` 僅可放入口結構與必要 Provider（例如 `QueryClientProvider`、全域 Theme/Context Provider）
3. `src/App.tsx` 禁止出現 `Route`、`Routes`、頁面級導頁邏輯，保持最小、乾淨、可讀
4. 若既有專案已將 Route 寫在 `App.tsx`，需在回覆中先提示重構建議與影響，再待使用者確認後調整
...
以下是好的 HTML 習慣，直接套用即可：

1. **語意化標籤**：按鈕用 `<button>`，導覽用 `<nav>`，表格用 `<table>`，不用 `<div>` 模擬互動元素
2. **圖片加 alt**：`<img>` 必須給 `alt`；純裝飾圖用 `alt=""`
3. **表單綁定 label**：每個 `<input>` / `<select>` / `<textarea>` 用 `<label htmlFor>` 明確綁定，或使用 `aria-label`
4. **正確標題層級**：頁面只有一個 `<h1>`，區塊標題依層級使用 `<h2>`、`<h3>`，不跳號

> 進階 ARIA、鍵盤導覽、焦點管理等屬於 `accessibility` Skill 的範疇，本 Skill 不處理。
...
>   #file:'.github/prototypes/Customer/index-list.html'
>   #file:'.github/prototypes/Customer/index-modal.html'
...
Step 4  產生或更新程式碼
  ...
  （Update 模式只改受影響檔案/區塊）
  ↓
Step 5  輸出 Router 變更提示（維持 Route 集中於 src/Router/index.tsx；App.tsx 不放 Route）
  ↓
...
- [ ] Route 是否僅存在於 `src/Router/index.tsx`？
- [ ] `src/App.tsx` 是否僅保留入口與必要 Provider（無 `Routes`/`Route`）？
...
| # | 類型 | 說明 | 影響範圍 | 建議處理 |
|---|------|------|----------|----------|
| 1 | 規格缺漏 | xxx 欄位未定義型別 | interface.ts | 詢問後端確認 |
| 2 | 命名衝突 | ... | ... | ... |

## 已自行處理的暫定方案

（若有）說明哪些問題採用了保守假設，標註「（待確認）」。
